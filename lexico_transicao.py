...
def transicoes():
    ...
    transicoes_existentes = [
        ...
        TransicoesAFD(N_TRANSICAO=88,TRANSICOES=[("}", 89), ("".join(char for char in ascii_characters if char not in ("}")), 88), ("\n", 88),],),
        ...
        TransicoesAFD(N_TRANSICAO=101,TRANSICOES=[(letras + numeros, 101), ("\n", 5), ("".join(char for char in ascii_characters if char not in (letras + numeros)),5,),],),
        TransicoesAFD(N_TRANSICAO=5, EH_FINAL=True, LOOKAHEAD=True, RETORNO=Token.ID),
        TransicoesAFD(N_TRANSICAO=53,TRANSICOES=[(numeros, 53), (".", 54), ("".join(char for char in ascii_characters if char not in (numeros + ".")), 55, ), ("\n", 55),],),
        TransicoesAFD(N_TRANSICAO=55, EH_FINAL=True, LOOKAHEAD=True, RETORNO=Token.CONST_INT),
        TransicoesAFD(N_TRANSICAO=54, TRANSICOES=[(numeros, 157)]),
        TransicoesAFD(N_TRANSICAO=157,TRANSICOES=[("\n", 58), (numeros, 157), ("E", 158), ("".join(char for char in ascii_characters if char not in (numeros + "E")), 58, ),],),
        TransicoesAFD(N_TRANSICAO=158, TRANSICOES=[("-", 159), (numeros, 12)]),
        TransicoesAFD(N_TRANSICAO=159, TRANSICOES=[(numeros, 12)]),
        TransicoesAFD(N_TRANSICAO=12,TRANSICOES=[(numeros, 12), ("".join(char for char in ascii_characters if char not in (numeros + ".")), 63, ), ("\n", 63),],),
        ...
        TransicoesAFD(N_TRANSICAO=75, TRANSICOES=[("=", 76)]),
